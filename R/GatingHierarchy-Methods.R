...
#' @importFrom RSVGTipsDevice setSVGShapeURL 
.renderNodes.svgAnno <- function(g) 
{
  ...
  for(i in 1:length(label)){
    ...
    if(thisPopId > 1){
      ...
      curPlotObj <- try(plotGate(gh, thisPopId,  arrange = FALSE), silent = TRUE)
...
#  browser()
  ...
  {	
    ...
    sfile <- tempfile(pattern = "gh", tmpdir = dir, fileext = ".svg")
...
#' 
#' @param x \code{GatingHierarchy} or \code{GatingSet} to be plotted
...
#'  plot(gs, dir = "myFolder") #each node link to multiple gates across samples
#' }
#' 
#' @rdname plot-methods
#' @aliases
