...
gs3 <- gs_clone(gs2)
Rm("singlets", gs3)
gs_add_gate(gs3, gs_get_gate(gs2, "CD3+"), parent = "not debris")
for(tsub in c("CD4", "CD8"))
  {
    gs_add_gate(gs3, gs_get_gate(gs2, tsub), parent = "CD3+")
    for(toAdd in gs_get_children(gs2, tsub))
    {
        thisParent <- gs_get_parent(gs2[[1]], toAdd,path="auto")
        gs_add_gate(gs3, gs_get_gate(gs2, toAdd), parent = thisParent) 
    }
  }
...
Rm("CD4", gs4)
# add it back
gs_add_gate(gs4, gs_get_gate(gs3, "CD4"), parent = "CD3+")
# add all the chilren back
for(toAdd in gs_get_children(gs3, "CD4"))
{
    thisParent <- gs_get_parent(gs3[[1]], toAdd)
    gs_add_gate(gs4, gs_get_gate(gs3, toAdd), parent = thisParent)
}
sampleNames(gs4) <- "4.fcs"

gs5 <- gs_clone(gs4)
# add another redundant node
gs_add_gate(gs5, gs_get_gate(gs, "CD4/CCR7+ 45RA+")[[1]], parent = "CD4")
gs_add_gate(gs5, gs_get_gate(gs, "CD4/CCR7+ 45RA-")[[1]], parent = "CD4")
...
```{r}
invisible(gs_set_node_visible(gs2, "singlets", FALSE))
plot(gs2)
plot(gs3)
```
...
```{r echo=FALSE}
#restore gs2
invisible(gs_set_node_visible(gs2, "singlets", TRUE))
```
