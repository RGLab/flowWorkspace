...
plotGate(gs[[1]], "CD4/38+ DR+", xbin = 128)      
# have the highest resolution by disabling hexbin 
plotGate(gs[[1]], "CD4/38+ DR+", xbin = 0)
```
...
```{r}
# hide the strip
plotGate(gs[[1]], "CD4/38+ DR+", strip = FALSE)      
# change the gating path length in strip 
plotGate(gs[[1]], "CD4/38+ DR+", path = "full")
plotGate(gs[[1]], "CD4/38+ DR+", path = 2)
```
### Change statistics
...
```{r}
# hide the pop stats
plotGate(gs[[1]], "CD4/38+ DR+", stats = FALSE)
...
plotGate(gs[[1]], "CD4/38+ DR+", raw.scale = FALSE)
# use the actual data range instead of instrument range
plotGate(gs[[1]], "CD4/38+ DR+", xlim = "data", ylim = "data")       
# zooming by manually set xlim and ylim
plotGate(gs[[1]], "CD4/38+ DR+", xlim = c(1000, 4000), ylim = c(1000, 4000))       
# hide the channel names and only show the marker names in x,y labs
plotGate(gs[[1]], "CD4/38+ DR+", marker.only = TRUE)
```
...
plotGate(gs[[1]], "CD4/CCR7- 45RA+"
              ...
              , overlay.symbol = list(`CCR7+ 45RA+` = list(fill = "red") #this overwrite the par.settings
                                      , `CD4/CCR7+ 45RA-` = list(fill = "darkgreen")
                                      )
              , key = list(text = list(c("CCR7+ 45RA+", "CCR7+ 45RA-")) # add legend
                          , points = list(col = c("red", "darkgreen"), pch = 19, cex = 0.5)
                          , columns = 2
                           )
         )
```
