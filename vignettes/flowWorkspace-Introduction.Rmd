...
## Purpose
The purpose of this package is to provide the infrastructure (`GatingSet`) to store, represent and exchange the gated flow data. By this we mean, accessing the samples, groups, transformations, compensation matrices, gates, and population statistics in the gating tree.

The `GatingSet` can be built from scratch within `R` or imported from `flowJo` XML workspaces (i.e. `.xml` or `.wsp` files) . We cannot import `.jo` files directly. You will have to save them in XML workspace format.
