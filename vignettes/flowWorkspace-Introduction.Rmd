---
title: 'flowWorkspace Introduction: A package to store and manipulate gated flow
  data'
...
output:
  ...
  html_document:
    ...
    toc_float: yes
vignette: |
  %\VignetteKeywords{flow cytometry, single cell assay, import} %\VignettePackage{flowWorkspace}   %\VignetteIndexEntry{flowWorkspace Introduction: A Package to store and maninpulate gated flow data} %\VignetteEngine{knitr}
---
...
The location and filename of the workspace XML file are printed along with a summary of its groups of samples. With the workspace file opened, some basic sample information can be accessed through some helper methods.
...
sampleNames(gs)
```
Note that this call to `sampleNames()` is different from the previous call to `getSamples()` on the workspace. The call to `getSamples()` on a `flowJoWorkspace` lists all samples stored in the XML file, while `sampleNames()` called on a `GatingSet` lists the ones actually getting parsed. Sometimes not all of samples in the `XML` will be imported for various reasons. Also we've seen an extra string `_xxx` is attached to the end of each sample name. It is due to the argument `additional.keys`, which has the default value set to `'$TOT'`. See more details on these parsing options from [How to parse a flowJo workspace](http://bioconductor.org/packages/release/bioc/vignettes/flowWorkspace/inst/doc/HowToParseFlowJo.html).

## Import gatingML
We currently support gatingML2.0 files exported from the `Cytobank` system. Parsing can be done with one convenient function, `parse.gatingML` from the `CytoML` package, that simply takes file paths of `gatingML` and `FCS`.
...
```
If you want to dive into the details and sub-steps of the parsing process, see the vignette of [CytoML](https://github.com/RGLab/CytoML).
...
```
But a `full` path may not be neccessary and could be too long to be visualized. So we provide the `path = 'auto'` option to determine the shortest path that is still unique within the gating tree.
...
nrow(cs[[1]])
```
Because `GatingSet` is a purely reference class, the class type returned by `getData` is a `cytoset`, which is the purely reference class analog of a `flowSet` and will be discussed in more detail below. Also note that the data is already compensated and transformed during the parsing.
...
head(getPopStats(gh))
```
Here `getPopStats` returns both the stats directly stored in the `flowJo` XML workspace and the stats calcuated by `GatingSet` through the gating. There could be minor differences between the two due to the numerical errors. However the differences should not be significant. Therefore this can be used as a validity check for the parsing accuracy. The `plotPopCV()` method allows for easy examination of the coefficient of variation between the XML and openCyto population statistics:
```{r plotPopCV}
plotPopCV(gh)
```
...
applied to different dimensions of the data. Dimensions of the data are appropriately transformed using the corresponding channel--specific functions from the list (like the one above).
...
Besides allowing definition all of these transformations by hand, we also provide some builtin transformations: `asinhtGml2_trans`, `flowJo_biexp_trans`, `flowJo_fasinh_trans` and `logicle_trans`. These are all very commonly used transformations in flow data analysis. The user can construct the transform object with one line of code. e.g.
...
```{r autoplot-multiple}
autoplot(gs, getChildren(gs[[1]], "rectangle")[1:4])
```
...
```{r load_cf}
files <- list.files(path, "Cyto", full.names = TRUE)
...
exprs(cf)[2,3]
```
To perform a shallow copy of an entire `cytoframe`, use the `shallow_copy` method rather than the `<-` operator. This will ensure that a new view is created to the full underlying dataset.
```{r shallow_cf}
cf1 <- shallow_copy(cf)
```
...
loaded <- load_cytoframe_from_h5(tmpfile)
```
...
sub_fr <- cs[[1]]
exprs(cs[[1]])[2,2]
exprs(sub_fr)[2,2] <- 0 # This WILL NOT affect the original data
exprs(cs[[1]])[2,2]
```

To circumvent this behavior and instead return a `cytoframe` that represents a view in to the data of the `original` cytoset, you need to use the `returnType` argument.

```{r extract2_cf}
sub_cf <- cs[[1, returnType = "cytoframe"]]
exprs(cs[[1]])[2,2]
exprs(sub_cf)[2,2] <- 0 # This WILL affect the original data
exprs(cs[[1]])[2,2]
```
...
Finally, the `shallow_copy()` and `realize_view()` methods work in a similar manner for `cytoset` objects as `cytoframe` objects. `shallow_copy()` will return a view in to the original data while `realize_view()` will perform a deep copy.
