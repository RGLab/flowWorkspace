...
gs
```
We have generated a `GatingSet` with `r length(gs)` samples, each  of which has `r length(gs_get_pop_paths(gs))-1` associated gates.
...
T <- gh_get_transformations(gh)
names(T)
T[[1]]
```

`gh_get_transformations` returns a list of functions to be
