...
```{r gh_get_gate}
node <- nodelist[3]
g <- gs_get_gate(gs, node)
g
```
We can retrieve the population statistics :
```{r getStats}
gs_get_pop_stats(gs)[1:10,]
```
...
```{r plotGate-multiple}
plotGate(gs,gs_get_children(gs[[1]], "rectangle")) 
```
